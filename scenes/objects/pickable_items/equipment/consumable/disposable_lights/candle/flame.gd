tool class_name VisualFlame extends MeshInstanceLayered
...
func _ready() -> void:
	randomize()
	material = get("material/0").duplicate() ; set("material/0", material)
	material.set_shader_param("noise_seed", rand_range(0,1000))


# Pauses the flame shader when parent Node paused
func _notification(what: int) -> void:
	if what == NOTIFICATION_PAUSED  : VisualServer.set_shader_time_scale(0.0) # RenderingServer in Godot 4
	if what == NOTIFICATION_UNPAUSED: VisualServer.set_shader_time_scale(1.0)
