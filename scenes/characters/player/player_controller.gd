...
#
#	if Input.is_action_pressed("playerhand|main_use_primary"):
...
#				return
#
##			_camera.set_crosshair_state("player|interact")
#
...
#				_click_timer = 0.0
#
#	if Input.is_action_just_pressed("playerhand|main_use_secondary") and drag_object != null:
...
#	direction.x += Input.get_action_strength("movement|move_right") - Input.get_action_strength("movement|move_left")
#	direction.z += Input.get_action_strength("movement|move_down") - Input.get_action_strength("movement|move_up")
#	direction = movement_basis.xform(direction)
#	direction = direction.normalized()*min(1.0, direction.length())
#
#	if Input.is_action_pressed("player|sprint") and stamina > 0 and GameManager.is_reloading==false:
#		direction *= 0.5;
#		change_stamina(-0.3)
#	else:
#		direction *= 0.2;
...
func handle_grab_input(delta : float):
	if is_grabbing:
		...
		wanna_grab = false
	if Input.is_action_pressed("player|interact") and is_grabbing == false:
...
#	else:
#		wanna_grab = false
#		if Input.is_action_just_released("player|interact") and grab_press_length >= hold_time_to_grab:
...
func update_throw_state(delta : float):
	match throw_state:
		ThrowState.IDLE:
			if Input.is_action_just_pressed("playerhand|main_throw") and owner.inventory.get_mainhand_item() and is_grabbing == false and GameManager.is_reloading == false:
				throw_item = ItemSelection.ITEM_MAINHAND
				throw_state = ThrowState.PRESSING
				throw_press_length = 0.0
			elif Input.is_action_just_pressed("playerhand|offhand_throw") and owner.inventory.get_offhand_item() and is_grabbing == false  and GameManager.is_reloading == false:
...
func handle_inventory(delta : float):
	...
	if Input.is_action_just_pressed("playerhand|cycle_offhand_slot") and GameManager.is_reloading == false:
...
#						item.call_deferred("global_translate", result.motion)
#
	if Input.is_action_just_released("player|interact") and not (wanna_grab or is_grabbing or interaction_handled):
