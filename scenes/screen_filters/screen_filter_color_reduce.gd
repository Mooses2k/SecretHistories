...
class_name ScreenFilterColorReduce

const SCREEN_FILTER_COLOR_REDUCE = preload("res://scenes/screen_filters/screen_filter_color_reduce.glsl")
const LOCAL_GROUP_SIZE := Vector3i(8, 8, 1)
const PUSH_CONSTANT_ELEMENT_COUNT = 3

@export_range(1, 8, 1) var color_depth : int = 5;
@export_range(0.0, 1.0, 0.01) var intensity : float = 1.0;
#@export_range(1.0, 4.0, 0.01) var gamma : float = 1.0;
...
func _init() -> void:
	...
	shader = rd.shader_create_from_spirv(SCREEN_FILTER_COLOR_REDUCE.get_spirv())
...
func _render_callback(p_effect_callback_type: int, render_data: RenderData) -> void:
	...
	push_constants.encode_s32(1*4, color_depth)
	#push_constants.encode_float(2*4, gamma)
